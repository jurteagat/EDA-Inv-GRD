...
# Run unit + integration tests (unit only, no Chrome needed)
Rscript -e 'testthat::test_dir("tests/testthat", filter = "helpers|datos|exportar")'

# Run all tests (requires Chrome/chromote for shinytest2)
...
Logic is extracted into testable pure functions in `R/`:

```
R/helpers.R    # label_var, fmt_soles, radios_log, theme_grd, paleta_grd, aggregations
R/datos.R      # descargar_si_falta, zero_pad_ubigeo, join_nombres_abreviados, construir_universo_comun
               # + cache helpers: ruta_cache_app, cache_app_vigente, guardar_cache_app, cargar_cache_app
R/exportar.R   # exportar_csv_geo, exportar_csv_temporal, renderizar_reporte_pdf
reporte_inversion.qmd  # Typst/Quarto template parametrizado (params: codigo, datos_rds)
global.R       # source(R/*), download-if-missing, pipeline completo, objetos compartidos
app.R          # UI bslib + server reactivo (usa helpers)
tests/testthat/test-helpers.R
tests/testthat/test-datos.R
tests/testthat/test-exportar.R
tests/testthat/test-app.R   # shinytest2 (requiere Chrome)
```

**Pestañas del navbar** (en orden): Resumen · Mapa · Distribuciones · Departamentos · Inversión específica · Ficha Técnica. La pestaña "Evolución temporal" fue eliminada; el gráfico `g_serie` (PIA/PIM/Devengado anual) vive ahora en la pestaña Resumen.

Key behaviors:
- `descargar_si_falta(id, destino)` descarga de Google Drive solo si el archivo no existe localmente. Los IDs están en `DRIVE_IDS` en `R/datos.R`.
- `zero_pad_ubigeo()` aplica str_pad a 6 dígitos; crítico para que `str_sub(ubigeo,1,2)` clasifique bien los depts 01-09.
- `join_nombres_abreviados()` hace coalesce de `nom_inv_corto` (curado) con `nombre_inversion` (fallback). Columna resultante: `nombre_abreviado`. Usada en popups, tablas, buscador y reporte PDF.
- Descarga de CSV y PDF envuelta en `shiny::withProgress`.
- Tema gráfico unificado `theme_grd()` aplicado a todos los ggplot.
- **Caché de arranque** (`midputs/rds/_cache_app.rds`): generado por `global.R` la primera vez; no se rastrea en git. En Posit Connect se reconstruye descargando desde Drive. Para forzar rebuild: `GRD_REBUILD_CACHE=1 Rscript -e 'source("global.R")'`.
- **KPIs monetarios** se formatean en millones con 1 decimal: `scales::label_number(scale = 1e-6, accuracy = 0.1, suffix = " M", prefix = "S/ ")`. No usar `fmt_soles()` para los value boxes del Resumen (reservado para otros contextos).
- **Mapa global — patrón `req(input$mapa_bounds)`**: el `observe` que agrega marcadores via `leafletProxy` debe comenzar con `shiny::req(input$mapa_bounds)`. Esto garantiza que el widget JS de Leaflet esté montado en el browser antes de enviarle comandos. NO usar `outputOptions(suspendWhenHidden = FALSE)` — causa un error JS (`null.calls`) porque el observe se dispara antes de que Leaflet inicialice.

## Data layout

- `raw/` — raw data files downloaded from MEF portal, suffixed `_YYYYMMDD` for versioning. Never manually edit these. **Not tracked in git** (archivos de cientos de MB; se descargan desde Drive con `descargar_si_falta()`).
- `midputs/` — intermediate datasets produced by this project. **Not tracked in git** salvo las excepciones abajo.
- `midputs/rds/nombres_abreviados.csv` — **sí rastreado**. Nombres curados keyed by `codigo_unico`; generated by `00_datos_entrada.qmd` from Google Sheets. Two columns: `codigo_unico` (character), `nom_inv_corto`.
- `midputs/rds/fechas_fuentes.csv` — **sí rastreado**. Metadatos de fecha de cada fuente de datos.
- `midputs/rds/_cache_app.rds` — **no rastreado**. Caché de arranque generado por `global.R`; se reconstruye desde Drive en el primer arrange de cada entorno.
...
YAML params on `00_datos_entrada.qmd` (relevant additions):
- `gsheet_id_nombres` — Google Sheet ID with curated abbreviated names (hoja `a_tranferir`, columnas `CODIGO_UNICO` + `NOM_INV_CORTO`). Sheet is public → read with `gs4_deauth()`, no OAuth needed.
- `gdrive_id_nombres_abrev` — Drive file ID for `nombres_abreviados.csv`; updated by Sección 3.
...
- `df_det_inv` carries a `nombre_abreviado` column after the join in chunk `leer-det-inv`. It coalesces `nom_inv_corto` (from Google Sheets) with `nombre_inversion` as fallback — never blank. All visualizations and tables use `nombre_abreviado`; `nombre_inversion` (the full MEF title) is preserved only for the investment detail table where both appear.
- The Google Sheet with abbreviated names has a typo in its tab name: it is `a_tranferir` (one 'r'), not `a_transferir`.
...
- **Mapa global — capas por tipología.** Agregar `addLayersControl(overlayGroups = tipologias_por_freq)` en `renderLeaflet`. En el `observe` que actualiza marcadores, iterar sobre `tipologias_por_freq` y llamar `clearGroup(p, tip)` + `addCircleMarkers(..., group = tip)` por cada tipología. Esto permite al usuario mostrar/ocultar cada tipología desde el control de capas.

- **Mapa departamental — escala fija de color.** Usar `leaflet::colorNumeric("RdYlGn", domain = c(0, 100))` con dominio fijo 0–100 para que el significado del color sea siempre consistente independientemente del filtro activo. Pasar `values = c(0, 100)` a `addLegend()`.

- **Gráficos temporales — años en eje X.** Siempre usar `scale_x_continuous(breaks = scales::breaks_width(1))` para mostrar todos los años. Usar `labs(x = NULL)` — no poner etiqueta "Año".
